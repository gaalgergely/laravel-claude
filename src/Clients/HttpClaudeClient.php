...
class HttpClaudeClient implements ClaudeClientContract
{
    ...
    public function __construct()
    {
        ...
        if (!is_string($this->config['api_key'] ?? null)) {
    ...
    {
        $betaVersion = $this->config['anthropic-beta'];
        if (!$betaVersion) return $this->client;
        return $this->client->withHeader('anthropic-beta', $betaVersion);
    }
}
